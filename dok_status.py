...
def create_csv_for_condition(df, condition_name, order_status, filename_suffix):
    ...
    print(f"--- 독독독 {condition_name} 상품 처리 시작 ---")
    ...
    if filename_suffix == "배송완료":
        csv_filename = f"CJGLS_sheet_upload_{filename_suffix}_독독독.csv"
    else:
        csv_filename = f"CJGLS_sheet_upload_{today_str}_{filename_suffix}_독독독.csv"
    
    csv_path = f"{DOWNLOAD_DIR}/{csv_filename}"
    csv_output.to_csv(csv_path, index=False, encoding="utf-8-sig")
    print(f"📦 독독독 {condition_name} CSV 저장 완료: {csv_path}")
